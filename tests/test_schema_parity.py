...
class SchemaParityTests(unittest.TestCase):
    ...
    def test_index_is_complete_bounded_and_pinned(self) -> None:
        ...
        self.assertEqual(len(list(FIXTURES.glob("*.valid.json"))), 32)
        self.assertEqual(len(list(FIXTURES.glob("*.invalid.json"))), 32)
        self.assertEqual(len(list(FIXTURES.glob("*.valid.json"))) + len(list(FIXTURES.glob("*.invalid.json"))), 64)
